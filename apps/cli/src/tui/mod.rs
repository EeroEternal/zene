...
use app::{App, ModelSelectorMode, PermissionPrompt, RunState};
    ...
    loop {
        ...
        if let Some(rx) = prompt_done_rx.as_ref() {
            if let Ok(result) = rx.try_recv() {
                ...
                active_cancel = None;
                match result {
                    Ok(text) => app.push_final_assistant(&text),
                    Err(err) => app.lines.push(app::ChatLine::Error(err.to_string())),
                }
                app.finish_turn();
                if !quiet_usage {
                    let usage = agent_rt
                        .block_on(async { agent.lock().await.turn_usage().clone() });
                ...
                match key.code {
                    ...
                    KeyCode::Enter => {
                        ...
                        if input == "/models" {
                            ...
                            if model_config::is_provider_configured(config) {
                                app.model_selector =
                                    Some(app::ModelSelector::for_models(config));
                            } else {
                                app.model_selector =
                                    Some(app::ModelSelector::for_providers(config));
                                app.lines.push(app::ChatLine::Assistant(
                                    "Provider not configured. Select a provider first.".to_string(),
                                ));
                            }
                            app.scroll_to_bottom();
                            continue;
                        ...
                        if input == "/model" {
                            let msg = {
                                let agent_guard = agent_rt.block_on(async { agent.lock().await });
                                model_config::models_help_message(&agent_guard)
                            };
...
fn cancel_turn(app: &mut App, active_cancel: &mut Option<CancellationToken>) -> bool {
    if matches!(
        ...
        if let Some(cancel) = active_cancel.as_ref() {
            ...
            app.request_cancel();
            app.lines
                .push(app::ChatLine::Status("Cancelling current turn…".to_string()));
            app.scroll_to_bottom();
            return true;
        }
    }
...
fn handle_running_enter(
    ...
    match steer_buffer.lock() {
        ...
        Err(_) => {
            app.lines.push(app::ChatLine::Error(
                "Failed to queue steer message.".to_string(),
            ));
        }
    }
    app.scroll_to_bottom();
    ...
    match code {
        ...
        KeyCode::Left | KeyCode::Right => {
            if selector.mode == ModelSelectorMode::SelectModel {
                ...
                let len = model_config::preset_models(provider).len();
                if len > 1 {
                    let delta = if code == KeyCode::Right { 1 } else { len - 1 };
                    selector.selected_model_index =
                        (selector.selected_model_index + delta) % len;
                    selector.model_name_override = None;
                }
            }
        }
            ...
            match mode {
                ...
                ModelSelectorMode::SelectProvider => {
                    if requires_key {
                        let has_key = {
                            let agent_guard = agent_rt.block_on(async { agent.lock().await });
                            model_config::has_api_key_for_provider(agent_guard.config(), provider)
                        };
