...
4. **Wave 12：Execution resume 与 Cloud transport（P1）**
   ...
   - 已增加 provider cursor 之后混合 platform/runtime event 的 canonical seq replay 测试，覆盖 cursor 重连后继续接收无 cursor platform event；EventOutbox 已迁移到独立 worker 模块，便于后续直接增加跨 worker restart/ack 集成测试。
   - 继续增加真实跨重连 replay/ack、worker restart 和 crash recovery 集成测试，并评估多 worker 共享持久化 outbox 的部署策略。
