...
>
> **进度快照：2026-08-13，基线 `a0f8139`（PR #63 已合并）。**
> 本文同时记录目标架构、已实现能力和剩余工作。
> Wave 16 审批 command 已中性。当前工作是事件 `event_type` 不再一律为 `acp`。
>
...
3. ACP、Cloud event 和 Core `AgentEvent` 仍存在语义转换。本地 `zene-runtime::RuntimeCommand` 与 Cloud `RuntimeCommand` 是两套类型。Cloud `event_type` 已是中性 kind；`payload` 仍为 ACP JSON 以兼容 Console 回放。
...
6. 权限已拆成纯 `evaluate` + 异步 `ApprovalBroker`。ACP 监听 `ApprovalRequested` 再发 `RuntimeCommand::Approval`。Cloud JobRunner 用同一套 `ApprovalDecision` 回复审批；ACP `optionId` JSON 只留在 RuntimeClient adapter。Cloud `event_type` 已分类；payload 仍是 ACP JSON。
...
5. **Cloud 审批 command 已中性化（Wave 16 第一刀）**：JobRunner 用 `ApprovalDecision` 回复审批，不再构造 ACP `outcome.optionId`。`event_type` 已是中性 kind；`payload` 仍是 ACP JSON。Cloud `RuntimeCommand` 仍不是 `zene-runtime::RuntimeCommand`。
...
| Wave 16 | 进行中 | 审批 command 与 `event_type` 已中性；payload 与整型 RuntimeCommand 仍待统一 |

### 当前收口状态与剩余边界

本轮已完成仓库内可以安全验证的主要优化，并保持旧协议兼容。当前剩余项分为三类：

- **本轮已完成的审批/事件收口**：runtime-owned waiter 与 Cloud `ApprovalDecision`；Cloud `event_type` 已是中性 kind，payload 仍为 ACP JSON 以兼容 Console。
...
4. **仍明确未自动完成的项目**
   ...
   - 本地与 Cloud 共用同一套 `RuntimeCommand` / `RuntimeEvent`；Cloud payload 不再以 ACP JSON 为产品语义（`event_type` 已中性）。
...
7. **Wave 16：统一 command/event（进行中）**
   ...
   - RuntimeClient 把 ACP `sessionUpdate` 分类为中性 `event_type`；payload 仍存原始 JSON 供 Console 回放。
...
| 选择 | Wave | 理由 |
| --- | --- | --- |
| 当前最大杠杆 | **Wave 16 payload** | `event_type` 已中性；Console 仍从 ACP JSON payload 渲染 |
...
- CLI 未开 waiter 时仍走旧同步 prompter。Cloud 审批回复已用中性 `ApprovalDecision`；`event_type` 已分类，payload 仍待去 ACP JSON。
