...
相关文档：[context-engine.md](./context-engine.md)、[agent-inference-context.md](./agent-inference-context.md)、[session-as-source-of-truth.md](./session-as-source-of-truth.md)（Session 事实 vs Context 投影）、[architecture.md](./architecture.md)（系统分层与 Crates 职责）、[research/pi-agent-harness.md](./research/pi-agent-harness.md)（Pi Harness 启发）、[archive/agent-runtime-optimization.md](./archive/agent-runtime-optimization.md)（归档的 Runtime 演进记录）。索引 / Repo Map 走工具侧，契约见 [context-engine.md §5](./context-engine.md#5-代码索引与-select)，不进 `zene-context`。
