...
相关文档：[context-engine.md](./context-engine.md)、[context-engine-projection.md](./context-engine-projection.md)（投影化优化）、[agent-inference-context.md](./agent-inference-context.md)、[session-as-source-of-truth.md](./session-as-source-of-truth.md)（Session 事实 vs Context 投影）、[pi-agent-harness-lessons.md](./pi-agent-harness-lessons.md)（Pi Harness 启发）。
