...
New Console capabilities follow the [feature slice](console-feature.md): domain type → db → `apps/api/src/features/<name>.rs` → `lib/cloud/<name>.ts` → hook → UI. Do not call `fetch` / `api()` with path strings from pages.
