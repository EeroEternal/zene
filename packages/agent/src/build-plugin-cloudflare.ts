  ...
  async onFiberRecovered(ctx) {
    ...
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
		...
		return `
// Auto-generated by @zene/agent build (cloudflare)
...
async function createLocalEnv() {
  throw new Error(
    "[zene] 'local' sandbox is not supported on Cloudflare Workers. " +
...
function assertAgentsDurabilityApi(doInstance, method) {
  if (typeof doInstance[method] !== 'function') {
		throw new Error(
			'[zene] The installed "agents" package does not provide the required Cloudflare Agents SDK method "' +
				method +
...
async function handleFlueFiberRecovered(ctx, _doInstance, agentName) {
  ...
  console.warn('[zene] Cloudflare fiber interrupted:', agentName, ctx.name, ctx.snapshot ?? null);
}
  ...
  try {
    ...
    if (isWebhook) {
      ...
      startWebhookFiber(doInstance, requestId, agentName, id, payload, handler, request).then(
        (result) => {
          console.log('[zene] Webhook handler complete:', agentName,
            result !== undefined ? JSON.stringify(result) : '(no return)');
        },
        (err) => {
          console.error('[zene] Webhook handler error:', agentName, err);
        },
      );
...
`;
	...
	async additionalOutputs(ctx: BuildContext): Promise<Record<string, string>> {
		...
		if (userConfigPath) {
			console.log(`[zene] Merging with user wrangler config: ${userConfigPath}`);
		}
		...
		if (sandboxClassNames.length > 0) {
			...
			for (const className of sandboxClassNames) {
				console.log(
					`[zene] Auto-wiring DO binding "${className}" to @cloudflare/sandbox's Sandbox class.`,
				);
			}
		}
...
function validateCloudflareAgentNames(ctx: BuildContext): void {
	...
	throw new Error(
		`[zene] Cloudflare target requires agent filenames to use lower-kebab-case so ` +
