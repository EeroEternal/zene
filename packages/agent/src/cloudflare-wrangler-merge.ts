...
export async function readUserWranglerConfig(root: string): Promise<UserConfigRead> {
	...
	try {
		...
		throw new Error(
			`[zene] Reading the Cloudflare wrangler config requires the "wrangler" package as a peer dependency.\n` +
	...
	try {
		...
		throw new Error(
			`[zene] Failed to read ${foundPath}: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
...
export function validateUserWranglerConfig(config: Record<string, unknown>): void {
	...
	if (Array.isArray(config.compatibility_flags)) {
		...
		if (!flags.includes(REQUIRED_COMPAT_FLAG)) {
			throw new Error(
				`[zene] Your wrangler config's "compatibility_flags" is missing "${REQUIRED_COMPAT_FLAG}". ` +
	...
	if (typeof config.compatibility_date === 'string') {
		...
		if (!/^\d{4}-\d{2}-\d{2}$/.test(userDate)) {
			throw new Error(
				`[zene] Your wrangler config's "compatibility_date" ("${userDate}") is not in YYYY-MM-DD format.`,
			);
		}
		if (userDate < MIN_COMPATIBILITY_DATE) {
			throw new Error(
				`[zene] Your wrangler config's "compatibility_date" is "${userDate}". ` +
...
export function assertSandboxPackageInstalled(
	...
	throw new Error(
		`[zene] Your wrangler config declares DO binding(s) whose class_name ends with "Sandbox" ` +
