...
export class Session implements FlueSession {
		...
		return createCallHandle(options?.signal, (signal) =>
			this.runOperation('skill', signal, async () => {
				...
				if (looksLikePath) {
					...
					if (!resolvedPath) {
						throw new Error(
							`[zene] Skill file "${name}" not found at ${skillsDirIn(this.env.cwd)}/${name} ` +
					...
					if (!this.config.skills[name]) {
						...
						throw new Error(
							`[zene] Skill "${name}" not registered. Available: ${available}.\n\n` +
	...
	private requireModel(model: Model<any> | undefined, callSite: string): Model<any> {
		...
		throw new Error(
			`[zene] No model configured for ${callSite}. ` +
	...
	private validateCustomToolNames(tools: ToolDef[]): void {
		...
		for (const toolDef of tools) {
			if (BUILTIN_TOOL_NAMES.has(toolDef.name)) {
				throw new Error(
					`[zene] Custom tool "${toolDef.name}" conflicts with a built-in tool. ` +
						`Built-in tools: ${[...BUILTIN_TOOL_NAMES].join(', ')}`,
				);
			}
			if (names.has(toolDef.name)) {
				throw new Error(
					`[zene] Duplicate custom tool name "${toolDef.name}". Tool names must be unique.`,
				);
			}
	...
	> {
		...
		if (!this.createTaskSession) {
			throw new Error('[zene] This session cannot create task sessions.');
		}
		if (this.taskDepth >= MAX_TASK_DEPTH) {
			throw new Error(`[zene] Maximum task depth (${MAX_TASK_DEPTH}) exceeded.`);
		}
	...
	private async runExclusive<T>(operation: string, fn: () => Promise<T>): Promise<T> {
		...
		if (this.activeOperation) {
			throw new Error(
				`[zene] Session "${this.id}" is already running ${this.activeOperation}. ` +
	...
	private assertActive(): void {
		if (this.deleted) {
			throw new Error(`[zene] Session "${this.id}" has been deleted.`);
		}
	}
	...
	private throwIfError(context: string): void {
		...
		if (errorMsg) {
			throw new Error(`[zene] ${context} failed: ${errorMsg}`);
		}
	}
