...
export class AgentClient implements FlueAgent {
	...
	private async openSession(
		...
		if (open) {
			if (mode === 'create') {
				throw new Error(`[zene] Session "${sessionId}" already exists for agent "${this.id}".`);
			}
			if (options?.role !== undefined && options.role !== open.role) {
				throw new Error(
					`[zene] Session "${sessionId}" is already open with ` +
		...
		if (mode === 'get' && !existingData) {
			throw new Error(`[zene] Session "${sessionId}" does not exist for agent "${this.id}".`);
		}
