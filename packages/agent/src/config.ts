 ...
 *
 * ```ts
 * import { defineConfig, defineOpenAICompletionsModel } from '@zene/agent/config';
 *
...
export function defineOpenAICompletionsModel(opts: {
	...
	if (typeof opts.baseUrl !== 'string' || opts.baseUrl.length === 0) {
		throw new Error(
			'[zene] defineOpenAICompletionsModel: `baseUrl` is required (e.g. "http://localhost:11434/v1").',
		);
	}
...
export function resolveConfigPath(opts: ResolveConfigPathOptions): string | undefined {
	...
	if (opts.configFile) {
		...
		if (!fs.existsSync(explicit)) {
			throw new Error(`[zene] Config file not found: ${opts.configFile}`);
		}
		return explicit;
	}
...
async function loadConfigModule(absConfigPath: string): Promise<unknown> {
	...
	try {
		...
		if (code === 'ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX') {
			throw new Error(
				`[zene] ${path.basename(absConfigPath)} uses TypeScript syntax that Node's ` +
		...
		if (code === 'ERR_UNKNOWN_FILE_EXTENSION') {
			...
			throw new Error(
				`[zene] Cannot load ${path.basename(absConfigPath)}: this Node ` +
...
export async function resolveConfig(opts: ResolveConfigOptions): Promise<ResolvedConfigResult> {
	...
	if (configPath) {
		...
		if (raw == null || typeof raw !== 'object') {
			throw new Error(
				`[zene] ${path.relative(cwd, configPath) || configPath} must export a config object as the default export.`,
			);
		}
	...
	if (!merged.target) {
		throw new Error(
			'[zene] Missing required `target`. Set it via `--target <node|cloudflare>` ' +
...
function formatValidationError(configPath: string, issues: readonly v.BaseIssue<unknown>[]): string {
	const lines = [`[zene] Invalid config in ${configPath}:`];
