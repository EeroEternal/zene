...
export class NodePlugin implements BuildPlugin {
	...
	generateEntryPoint(ctx: BuildContext): string {
		...
		return `
// Auto-generated by @zene/agent build (node)
  ...
  if (isWebhook) {
    ...
    handler(ctx).then(
      (result) => {
        ...
        console.log('[zene] Webhook handler complete:', name, result !== undefined ? JSON.stringify(result) : '(no return)');
      },
      (err) => {
        ctx.setEventCallback(undefined);
        console.error('[zene] Webhook handler error:', name, err);
      },
    );
...
console.log('[zene] Server listening on http://localhost:' + port);
if (isLocalMode) {
  console.log('[zene] Mode: local (all agents invokable, including trigger-less)');
  console.log('[zene] Available agents: ' + ${JSON.stringify(agents.map((a) => a.name).join(', '))});
} else {
  console.log('[zene] Available agents: ' + ${JSON.stringify(webhookAgents.map((a) => a.name).join(', '))});
}
