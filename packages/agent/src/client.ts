	...
	const ctx: FlueContextInternal = {
		...
		async init(options?: AgentInit): Promise<FlueAgent> {
			if (!options || !('model' in options)) {
				throw new Error(
					'[zene] init() requires a model. Pass { model: "provider/model-id" } or { model: false }.',
				);
			}
			if (options.model !== false && typeof options.model !== 'string') {
				throw new Error('[zene] init({ model }) must be a model string or false.');
			}

			const id = options.id ?? config.id;
			if (initializedAgentIds.has(id)) {
				throw new Error(`[zene] init() has already been called for agent "${id}" in this request.`);
			}
...
async function resolveSessionEnv(
	...
	if (isBashLike(sandbox)) {
		throw new Error(
			'[zene] init({ sandbox }) no longer accepts a Bash-like object directly. ' +
	...
	throw new Error('[zene] Invalid sandbox option passed to init().');
}
