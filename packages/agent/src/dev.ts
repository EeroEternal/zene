...
export async function dev(options: DevOptions): Promise<void> {
	...
	for (const f of envFiles) {
		console.error(`[zene] Loading env from: ${f}`);
	}
	...
	console.error(`[zene] Starting dev server (target: ${options.target})`);
	console.error(`[zene] Watching: ${root}`);
	console.error(`[zene] Building...`);
	...
	try {
		...
		throw new Error(
			`[zene] Initial build failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	console.error(`[zene] Built in ${Date.now() - initialStart}ms`);
	...
	if (reloader.url) {
		console.error(`[zene] Server: ${reloader.url}`);
		const exampleAgent = pickExampleAgentName(output, root);
		if (exampleAgent) {
			console.error(`[zene] Try: curl -X POST ${reloader.url}/agents/${exampleAgent}/test-1 \\`);
			console.error(`         -H 'Content-Type: application/json' -d '{}'`);
		}
	}
	console.error(`[zene] Press Ctrl+C to stop\n`);
	...
	const watcher = createWatcher({
		...
		onChange: (relPath) => {
			...
			console.error(`[zene] Change detected: ${relPath}`);
	...
	const shutdown = async (signal: string, exitCode: number) => {
		...
		shuttingDown = true;
		console.error(`\n[zene] Received ${signal}, shutting down...`);
		watcher.close();
		try {
			await reloader.stop();
		} catch (err) {
			console.error(
				`[zene] Error during shutdown: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
		console.error(`[zene] Stopped.`);
...
function createWatcher(options: WatcherOptions): WatcherHandle {
	...
	try {
		...
		console.error(
			`[zene] Failed to watch ${root}: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	private async spawnAndWait(): Promise<void> {
		...
		const pipe = (data: Buffer) => {
			...
			for (const line of text.split('\n')) {
				...
				if (
					line.includes('[zene] Server listening') ||
					line.includes('[zene] Available agents:') ||
					line.includes('[zene] Mode: local')
				) {
					continue;
				}
				console.error(line);
		...
		child.on('exit', (code, signal) => {
			if (this.child === child) {
				...
				if (code !== 0 && code !== null) {
					console.error(
						`[zene] Node server exited unexpectedly (code=${code}, signal=${signal ?? 'none'})`,
					);
				}
			}
		});
...
async function createCloudflareReloader(opts: {
	...
	try {
		...
		throw new Error(
			`[zene] Cloudflare dev requires the "wrangler" package as a peer dependency.\n` +
...
class CloudflareReloader implements DevReloader {
	...
	async reload(buildChanged: boolean): Promise<void> {
		...
		if (!buildChanged) {
			console.error(`[zene] No structural change — wrangler will hot-reload\n`);
			return;
		}
	...
	private async startWorker(): Promise<void> {
		if (!fs.existsSync(this.configPath)) {
			throw new Error(
				`[zene] Expected ${this.configPath} after build, but it doesn't exist. ` +
		...
		// hung server. We re-emit at `console.error` with a `[zene]` prefix.
		//
		...
		this.errorListener = (event: WranglerErrorEvent) => {
			...
			const causeMsg =
				cause && typeof cause === 'object' && 'message' in cause
					...
					: undefined;
			console.error(`[zene] Wrangler error (${event?.source ?? 'unknown'}): ${reason}`);
			if (causeMsg) console.error(`[zene]   ${causeMsg}`);
		};
	...
	private async disposeWorker(): Promise<void> {
		...
		try {
			...
			console.error(
				`[zene] Error disposing Cloudflare worker: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
	}
}
 ...
 *
 * Throws a friendly `[zene]`-prefixed error if any path doesn't exist. The
...
export function resolveEnvFiles(envFiles: string[] | undefined, cwd: string): string[] {
	...
	return envFiles.map((p) => {
		...
		if (!fs.existsSync(abs)) {
			throw new Error(`[zene] --env points at a path that doesn't exist: ${p}`);
		}
		return abs;
	});
}
