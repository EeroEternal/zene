//! Actor-style control plane for a long-lived agent runtime.
//!
//! `RuntimeHandle` is the only mutable control surface exposed to transports.
//! The actor owns the `Agent`, so session and turn state cannot be concurrently
//! mutated by ACP, Cloud, or local callers.
